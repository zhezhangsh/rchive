...
url <- paste('https://www.encodeproject.org/targets/', sapply(strsplit(nms, ' '), function(x) x[1]), '-human', sep='');
all <- do.call('c', all); 
names(all) <- ids;
df <- data.frame(Name=nms, URL=url, stringsAsFactors = FALSE);
rownames(df) <- ids;
df <- df[order(rownames(df)), ];
all <- all[rownames(df)]; 
saveRDS(all, paste(path, 'r', 'tf_targets.rds', sep='/')); 
saveRDS(df, paste(path, 'r', 'tf_anno.rds', sep='/')); 
