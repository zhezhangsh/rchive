...
#########################
PrepareMetaGwas<-function(phred_tbls, patch.only=FALSE,
  ...
  # phred_tabls Name:description pairs of Phred score tables
  # pathc.only  Only run the patch script if TRUE
  ...
  } # Finish non-patch run 
  
  #############################################################################################################################
  #############################################################################################################################
  ###### Run patches to fix extra issues
  ###### Based on all files having been created as above
  std<-readRDS(paste(path.out, 'study.rds', sep='/'));  
  std.by.id<-readRDS(paste(path.out, 'study_by_id.rds', sep='/'));
  key2std<-readRDS(paste(path.out, 'keyword2study.rds', sep='/'));
  key2pub<-readRDS(paste(path.out, 'keyword2pubmed.rds', sep='/'));
  std.by.id<-lapply(std.by.id, function(s) {
    pubs<-sort(s[['PubMed']], decreasing=TRUE);
    keys<-rownames(key2std)[key2std[, s[['ID']]]>0];
    n<-sapply(pubs, function(p) length(intersect(keys, rownames(key2pub)[key2pub[, p]>0])));
    append(s, c("Representative PubMed"=pubs[n==max(n)][1]), which(names(s)=='PubMed')-1);   
  });
  rep.pub<-sapply(std.by.id, function(s) s[['Representative PubMed']])[rownames(std)];
  rep.pub[is.na(rep.pub)]<-'';
  std$Representative_PubMed<-rep.pub
  std<-std[, c(1:3, ncol(std), 4:(ncol(std)-1))];
  saveRDS(std, paste(path.out, 'study.rds', sep='/'));
  saveRDS(std.by.id, paste(path.out, 'study_by_id.rds', sep='/'));
  
  # Assign PubMed ID to analysis through study if the analysis has no PubMed
  pub<-readRDS(paste(path.out, 'pubmed.rds', sep='/'));
  ana<-readRDS(paste(path.out, 'analysis.rds', sep='/'));
  ana.by.id<-readRDS(paste(path.out, 'analysis_by_id.rds', sep='/'));
  ana2pm<-readRDS(paste(path.out, 'analysis2pubmed.rds', sep='/'));
  pm2ana<-readRDS(paste(path.out, 'pubmed2analysis.rds', sep='/'));
  
  ids<-names(ana2pm);
  ana2pm<-lapply(ids, function(id) {
    pubs<-ana2pm[[id]];
    pubs<-sort(unique(pubs[pubs!='-1' & pubs!='0' & !is.na(pubs)]));
    if (length(pubs) == 0) std[as.vector(ana[id, 'Study_ID']), 'Representative_PubMed'] else pubs;
  });
  names(ana2pm)<-ids;
  
  ana.by.id<-lapply(ana.by.id, function(a) {
    a$PubMed<-ana2pm[[a[['ID']]]]; 
    a;
  });
  
  rep.pub<-sapply(ana2pm, function(x) x[length(x)])[rownames(ana)];
  rep.pub[is.na(rep.pub)]<-'';
  ana[, 'PubMed_ID']<-rep.pub;
  
  pm<-unlist(ana2pm, use.names=FALSE);
  an<-rep(ids, sapply(ana2pm, length));
  pm2ana<-lapply(split(an, pm), function(x) sort(unique(x)));
  
  n<-sapply(pm2ana, length)[rownames(pub)];
  n[is.na(n)]<-0;
  pub[, 'Num_Analysis']<-n;
  
  saveRDS(pub, file=paste(path.out, 'pubmed.rds', sep='/'));
  saveRDS(ana, file=paste(path.out, 'analysis.rds', sep='/'));
  saveRDS(ana.by.id, file=paste(path.out, 'analysis_by_id.rds', sep='/'));
  saveRDS(ana2pm, file=paste(path.out, 'analysis2pubmed.rds', sep='/'));
  saveRDS(pm2ana, file=paste(path.out, 'pubmed2analysis.rds', sep='/'));
